...
typedef multimap<int, Point> Points;
bool ReadPointsFromFile(ifstream &input, unsigned int countLines, Points &points)
{
	for (unsigned int it = 0; it < countLines; ++it)
	{
		...
		{
			point1.numberOfLine = it;
			point2.numberOfLine = it;
			...
			points.emplace(pair<int, Point>(x1, point1));
			points.emplace(pair<int, Point>(x2, point2));
		}
	}
	return true;
}
int main()
{
	...
	{
		cout << it.first << ' ' << it.second.numberOfLine << ' '<< it.second.position << endl;
	}
    return 0;
}
